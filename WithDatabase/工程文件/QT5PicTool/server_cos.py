...
import serve_shelve
from serve_shelve import getShelve, setShelve, resetDB
...
class CosServer:
    ...
    def upload_file(self, file: str) -> Dict[str, str]:
        # 强烈建议您以二进制模式(binary mode)打开文件,否则可能会导致错误
        with open(file, 'rb') as fp:
            dataStr = str(time.strftime('%Y-%m-%d',time.localtime(time.time())))
            filename = Key = "img/" + dataStr + "/" + str(uuid.uuid4()) + os.path.split(file)[1]
        ...
        if(len(tag)) :
            return {
                "root": "https://" + self.__config.bucket + ".cos.ap-beijing.myqcloud.com/",
                "path": "/img/" + dataStr,
                "file": filename,
                "url": "https://" + self.__config.bucket
